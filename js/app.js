...
/** jQuery Mobile input component. */
var JQueryMobileInput = React.createClass({
  displayName: 'JQueryMobileInput',
  ...
  render: function() {
    return React.DOM.div({className: 'form-group'},
      React.DOM.label({for: 'usr'}, 'Name:'),
      React.DOM.input({className: 'form-control', type: 'text', id: 'usr', onKeyDown: this.onKeyDown})
    );
  }
});
JQueryMobileInput = React.createFactory(JQueryMobileInput);

/** jQuery Mobile select component. */
var JQueryMobileSelectInput = React.createClass({
  displayName: 'JQueryMobileSelectInput',
  ...
  render: function() {
    return React.DOM.div({className: 'form-group'},
      React.DOM.label({for: 'sel1'}, 'Name:'),
      React.DOM.select({className: 'form-control', id: 'sel1', onChange: this.redirect}, this.props.options.map( function(option) { return React.DOM.option(null, option) }))
    );
  }
});
JQueryMobileSelectInput = React.createFactory(JQueryMobileSelectInput);
...
var PageOneContent = React.createClass({
  ...
  render: function() {
    return React.DOM.div(null,
      ...
      JQueryMobileInput({href: '#two'}),
      JQueryMobileSelectInput({href: '#two', options: ["1","2","3","4"]}),
      // JQueryMobileInput({onSubmit: "window.location='#two'"}, 'Input'),
...
var PageTwoContent = React.createClass({
  ...
  onKeyDown: function(event) {
    var callback = function (xml) { console.log('check dis xml', xml)};
    var service = `online/search?keyword=${event.target.value}`;
  ...
  render: function() {
    return React.DOM.div(null,
      ...
      JQueryMobileInput({href: '#one', onKeyDown: this.onKeyDown}),
      JQueryMobileSelectInput({href: '#one', options: ["1","2","3","4"]})
    );
  }
});
